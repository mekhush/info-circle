    ...
    <div className="bg-white/40 backdrop-blur-xl border border-white/60 rounded-2xl shadow-xl p-5 mb-6">
      ...
      {!expanded && (
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/40">
          <button
            onClick={() => setExpanded(true)}
            className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium text-blue-600 hover:bg-blue-50 transition-colors"
          >
            ❓ Ask Question
          </button>
          <button
          ...
          <button
            onClick={() => setExpanded(true)}
            className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium text-pink-600 hover:bg-pink-50 transition-colors ml-auto"
          >
            📝 Write Article
          </button>
        </div>
...
const CommentSection = ({ post, user }) => {
  const [comments, setComments]   = useState(Array.from(post.comment || []));
          ...
          <div className="flex-1 max-w-md mx-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-full border border-gray-200 bg-white/80 text-gray-700 placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all"
              />
              {searchQuery && (
          ...
          `}>
            ...
            <div className="bg-white/40 backdrop-blur-xl border border-white/60 rounded-2xl shadow-lg p-4">
              ...
              {[
                ...
                { icon: '✍️', label: 'Ask Question', href: '#composer', scroll: true },
